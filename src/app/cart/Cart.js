...
export default function Cart() {
   ...
   const cartItems = useSelector((state) => state.ProductsSlice)
  ...
  {cartItems.map((item, index) => (
    ...
    <div key={index} className="bg-white p-4 w-[800px] rounded-xl">
      <table className="w-full bg-white rounded-xl">
        <thead>
          <tr className="text-center border-b border-gray-400 w-full text-[#7f7f7f] text-sm font-medium uppercase leading-[14px] tracking-wide">
            <th className="text-left px-2 py-2">{item.title}</th>
